...
2. Configure tmux and agents to use the tool.
...
**2. configure tmux and agents to use the cli tool**
...
It detects which agents you have, offers them as a checklist, edits your tmux configuration, and checks the result against a throwaway tmux server.
It asks before every change and backs up every file it edits.
If a file cannot be edited safely, like one generated by home-manager or Nix, it is reported with the content to be inserted.

Some useful flags. `tmux-agent-status install --help` lists them all:

| Flag | Does |
| --- | --- |
| `--dry-run` | print the plan and change nothing |
| `-y`, `--yes` | take the recommended answer to every question |
| `--agents` | only install agent hooks, leaving your tmux configuration alone |
| `--agents=codex,cursor` | only selected agents. The `=` is required |
| `--tmux-hook` | only install the tmux hooks |
| `--tmux-format` | only install the tmux window format |
| `--no-tmux-format` | everything except the tmux window format |

Start with `tmux-agent-status install --dry-run` if you would like to read the plan first.

If you would rather configure yourself, the steps 2.1 to 2.3 are written out below:
2. tmux hooks: include the `tmux-agent-status.conf` into your tmux config to hook onto tmux's events.
3. tmux window format: include the `agent_status` placeholder in your tmux's window status format.
4. agent hooks: register the `tmux-agent-status` commands as agent hooks.

**2.1. Tmux hooks: source the tmux snippet.**
...
**2.2. Tmux window format: Include the glyph term in your tmux window format.**
...
**2.3. Agent hooks: capture agent events.**
...
`tmux-agent-status install --dry-run` reports the state of steps 2 to 4 without changing anything,
which is the quickest and most easy check.
