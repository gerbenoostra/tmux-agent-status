...
Step 1 is yours; pick a route in [docs/install.md](docs/install.md). Steps 2 to 4 are what the
`install` subcommand does:

```sh
tmux-agent-status install
```

It detects which agents you have, offers them as a checklist, edits your tmux configuration, and
checks the result against a throwaway tmux server before letting it stand. It asks before every
change and backs up every file it edits. A file it cannot edit safely - one generated by
home-manager or Nix, say - is reported with the line to paste rather than touched.

Some useful flags. `tmux-agent-status --help` lists them all:

| Flag | Does |
| --- | --- |
| `--dry-run` | print the plan and change nothing |
| `-y`, `--yes` | take the recommended answer to every question |
| `--agents` | only step 4, leaving your tmux configuration alone |
| `--agents=codex,cursor` | only those agents. The `=` is required; `--agents codex` is a usage error, because a bare `--agents` has to be able to stand on its own |
| `--tmux-hook` | only step 2 |
| `--tmux-format` | only step 3 |
| `--no-tmux-format` | everything except step 3 |

Start with `tmux-agent-status install --dry-run` if you would like to read the plan first. If you
would rather paste every line yourself, steps 2 to 4 are written out below and stay supported.
...
which is the quickest check of the lot.
