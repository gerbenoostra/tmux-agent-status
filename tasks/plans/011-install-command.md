...
2. **Refuse what must not be edited.** Not a regular file (directory, fifo, socket, device): refuse.
   On a read-only filesystem or inside a package store: refuse with the generated-config message
   below. Outside `$HOME`: warn and ask, since a dotfiles checkout may legitimately live elsewhere.
   More than one hard link: warn and ask, because the rename breaks the link and a dotfiles setup
   that hardlinks would silently diverge. `-y` proceeds on both warnings and says so.
...
### Generated configs are refused, not edited

A config that resolves into a read-only store (`/nix/store`, a Guix store, a read-only mount) is not
a file anyone may edit: the next `home-manager switch` reverts it, and the tool would have taught
the user a lie. Detect it, refuse the step, and print the fragment to add to the generator instead -
the `source-file` line, or the format string with the term already spliced in. Doing the merge and
printing the result is most of the value anyway.

`docs/install.md` already documents the nix route; this is the same advice, delivered at the moment
the user needs it.
...
it afterwards. On tmux 3.6a this returns `#I:#W#{?window_flags,#{window_flags}, }`, which is also
the hard-coded fallback if tmux cannot be run at all. Propose two new `set -g` lines built from it.
...
8. **A target under a read-only store path**: refused, and the printed fragment is the one the user
   needs.
...
22. A home-manager-generated `~/.tmux.conf`, confirming the refusal fires and prints something the
