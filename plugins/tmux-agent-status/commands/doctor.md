...
includes `tmux-agent-status register`: name it as the command the user can run, and never run it
...
- Not found: the hooks are firing and exiting silently. Point at `docs/install.md`, and note that
  ...
  always the `PATH` of an interactive shell. There is nothing `tmux-agent-status register` can do
...
- `focus-events` not `on`: `pane-focus-in` still fires on the first client attach to a session (F9),
...
- Missing from either: the glyph is invisible on exactly the windows using that format - a term
  ...
  `tmux-agent-status register --tmux-format` does that edit: it finds the line that wins, splices the
...
`tmux-agent-status register` writes either, so do not offer it here. Passing this check means
...
- Present: the user has both the plugin and the manual paste, so every event fires twice. The writes
  ...
  `tmux-agent-status register` for it: that command never deletes from that file either.
...
`tmux-agent-status register --dry-run` reports them together and `tmux-agent-status register` repairs
