...
/// Where the shipped files are listed, one row per installation method.
///
/// Printed rather than described because the answer is different for every
/// install route - a nix package, a profile, a tarball, a checkout - and the
/// one thing this process cannot work out is which route put a read-only file
/// where it found it. The table names both the tmux snippet and the agent
/// drop-ins, which is every file this error can be about.
///
/// `main` rather than a tag: the page documents the tool rather than this
/// build, and a link to a tag that was never cut is worse than one that tracks
/// the docs. `tests/source_invariants.rs` checks the heading still exists.
const MANUAL_FILES: &str = "https://github.com/gerbenoostra/tmux-agent-status/blob/main/\
                            docs/install.md#manual-file-reference";
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::Unwritable { path, directory: _ } if in_a_package_store(path) => write!(
                f,
                "{} is generated by nix and its directory is read-only.\n\
                 Make the change where the file is generated, not here. What it should \
                 contain is listed per installation method at\n  {MANUAL_FILES}",
                path.display()
            ),
